...
const std::array<Score, 7> see_values = { 100, 300, 300, 450, 900, 10000, 0 };

const std::array<BitBoard, 4> castling_relevant_bbs = {
...
const std::array<BitBoard, 4> castling_check_bbs = {
...
const std::array<BitBoard, 4> castling_empty_bbs = {
...
const BitBoard all_castling_squares = castling_relevant_bbs[0] | castling_relevant_bbs[1]
...
std::vector<Move> Board::GetMoves(const BitBoard critical) {
  const MoveGenType move_gen_type = static_cast<MoveGenType>(_move_gen_type);
  ...
  if (move_gen_type == MoveGenType::Fast) {
	for (auto move_it = moves.begin(); move_it != moves.end(); ) {
	  ...
	  if (turn == kWhite && BOT_COLOR) {
		if (row > 2) {
		  move_it = moves.erase(move_it);
		  continue;
		}
	  }
	  else if (turn == kBlack && !BOT_COLOR) {
		if (row < 5) {
		  move_it = moves.erase(move_it);
		  continue;
		}
  ...
  for (auto move_it = legal_moves.begin(); move_it != legal_moves.end(); ) {
	int32_t row = GetSquareY(GetMoveDestination(*move_it));
	if (turn == kWhite && BOT_COLOR) {
	  if (row > 2) {
		move_it = legal_moves.erase(move_it);
		continue;
	  }
	}
	else if (turn == kBlack && !BOT_COLOR) {
