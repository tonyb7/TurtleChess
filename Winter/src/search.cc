...
Move RootSearch(Board &board, Depth depth, Milliseconds duration = Milliseconds(24 * 60 * 60 * 1000)) {
  ...
  if (BOT_COLOR == -1) {
	std::ifstream engine_color_file(
		"/Users/tonyb/Desktop/chess/lichess-bot/engines/engine_color.txt");
...
//  std::cout << "moves size: " << moves.size() << '\n';
  for (auto move_it = moves.begin(); move_it != moves.end(); ) {
	int32_t row = GetSquareY(GetMoveDestination(*move_it));
//	std::cout << "Move " << *move_it << " to dest " << GetMoveDestination(*move_it);
//	std::cout << " which is on row " << row << '\n';
//	std::cout << board.get_turn() << " to move and BOT_COLOR = " << BOT_COLOR << '\n';
	if (board.get_turn() == kWhite && BOT_COLOR) {
//	  std::cout << "CONSIDER WHITE" << std::endl;
	  if (row > 2) {
//		std::cout << "ERASE WHITE" << std::endl;
		move_it = moves.erase(move_it);
		continue;
	  }
	}
	else if (board.get_turn() == kBlack && !BOT_COLOR) {
//	  std::cout << "CONSIDER BLACK" << std::endl;
	  if (row < 5) {
//		std::cout << "ERASE BLACK" << std::endl;
		move_it = moves.erase(move_it);
		continue;
	  }
	}
...
//  std::cout << "Finished filtering. Moves left: " << moves.size() << '\n';
  ...
  // Filter non turtle moves
//  std::cout << "Filtering moves end\n";
//  std::cout << "moves size: " << Threads.main_thread->moves.size() << '\n';
  for (auto move_it = Threads.main_thread->moves.begin();
	   move_it != Threads.main_thread->moves.end(); ) {
	int32_t row = GetSquareY(GetMoveDestination(*move_it));
...
//		std::cout << "ERASE WHITE" << std::endl;
		...
		move_it = Threads.main_thread->moves.erase(move_it);
		continue;
	  }
	}
	else if (board.get_turn() == kBlack && !BOT_COLOR) {
...
//  std::cout << "Finished filtering. Moves left: " << Threads.main_thread->moves.size() << '\n';
//  std::cout << "Move " << Threads.main_thread->moves[0] << " to dest " << GetMoveDestination(Threads.main_thread->moves[0]);
//  std::cout << " which is on row " << GetSquareY(GetMoveDestination(Threads.main_thread->moves[0])) << '\n';
//  std::cout << board.get_turn() << " to move and BOT_COLOR = " << BOT_COLOR << '\n';
//
//  std::cout << "Returning " << parse::MoveToString(Threads.main_thread->moves[0]);
